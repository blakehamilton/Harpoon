...
This is a modified version of android.support.multidex.MultiDex
...
public final class Harpoon {
    static final String TAG = "Harpoon";
     ...
     *         extension.
     */
    public static void inject(Context context, List<File> dexDirs) {

// blake: Don't need this because we're not extracting Dex files from our own APK
//        Log.i(TAG, "inject");
//        if (IS_VM_MULTIDEX_CAPABLE) {
//            Log.i(TAG, "VM has multidex support, Harpoon support library is disabled.");
//            return;
//        }
        ...
        try {
            ...
            synchronized (installedApk) {
                ...
                if (Build.VERSION.SDK_INT > MAX_SUPPORTED_SDK_VERSION) {
                    Log.w(TAG, "Harpoon is not guaranteed to work in SDK version "
                ...
                try {
                    ...
                    Log.w(TAG, "Something went wrong when trying to clear old Harpoon extraction, "
                            + "continuing without cleaning.", t);
                }

                File dexDir = new File(applicationInfo.dataDir, SECONDARY_FOLDER_NAME);

                // copy the passed in list of files
                List<File> apkDirs = new ArrayList<>();
                apkDirs.addAll(dexDirs);

                for(File apk: dexDirs){
                    // if needed, extract multidex dex files and add them to this list
                    // in this case we want to set forceReload to TRUE always, in case the external APK is different, which we don't know
                    List<File> extractedDexFiles = MultiDexExtractor.loadBH(context, applicationInfo, dexDir, apk, true);
                    for(File f : extractedDexFiles){
                        Log.d(TAG, String.format("MultiDexExtractor produced: %s", f.getAbsolutePath()));
                        apkDirs.add(f);
                    }
                }

                if (checkValidZipFiles(apkDirs)) {
                    installSecondaryDexes(loader, dexDir, apkDirs);
                } else {
