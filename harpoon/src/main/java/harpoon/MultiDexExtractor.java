...
final class MultiDexExtractor {
    private static final String TAG = MultiDexExtractor.class.getName();
    /**
     ...
     *         secondary dex files
     */
    static List<File> loadBH(Context context, ApplicationInfo applicationInfo, File dexDir, File sourceApk,
                           boolean forceReload) throws IOException {
        Log.i(TAG, "MultiDexExtractor.load(" + applicationInfo.sourceDir + ", " + forceReload + ")");
        // blake: we pass in the APK file path instead of assuming its our APK
        //final File sourceApk = new File(applicationInfo.sourceDir);
        long currentCrc = getZipCrc(sourceApk);
        List<File> files;
        if (!forceReload && !isModified(context, sourceApk, currentCrc)) {
